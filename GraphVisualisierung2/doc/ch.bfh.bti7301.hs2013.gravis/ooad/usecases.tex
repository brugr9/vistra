...
\begin{usecase}{Import Graph}
    ...
    \mainsuccess{
	    ...
	    \item Der Default-Graph wird ausgew\"ahlt (select, siehe \textit{UC Select Graph} Seite~\pageref{uc:Select Graph}).
    }
\end{usecase}
\newpage 
% 
...
\begin{usecase}{Delete Graph}
    ...
    \mainsuccess{
	    ...
	    \item Der gel\"oschte Graph wird aus der Graph-Parameterliste entfernt (remove).
	    \item Der Default-Graph wird ausgew\"ahlt (select, siehe \textit{UC Select Graph} Seite~\pageref{uc:Select Graph}).
    }
\end{usecase}
\newpage 
...
\begin{usecase}{Delete Algorithm}
    ...
    \mainsuccess{
	    ...
	    \item Der User wird dazu aufgefordert, den Pfad und den Dateinamen der Datei anzugeben (open) oder den Vorgang abzubrechen (abort).
	    \item Die angegebene Datei wird aus der Dateistruktur des Systems gel\"oscht (delete).
	    \item Der gel\"oschte Algorithm wird aus der Algorithm-Parameterliste entfernt (remove).
	    \item Der Default-Graph wird ausgew\"ahlt (select, siehe \textit{UC Select Graph} Seite~\pageref{uc:Select Graph}).
    }
\end{usecase}
\newpage 
